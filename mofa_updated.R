...
COUNTS_DIR <- paste0(BASE_DIR, "/RNA_seq/counts")
OUTDIR     <- paste0(BASE_DIR, "/mofa_output")
...
paired <- read.csv(paste0(BASE_DIR, "/WGBS/mofa_paired_samples.csv"))
...
# ── Convert BOTH RNA and methylation gene symbols to ENSEMBL IDs ───
# (previously only methylation was converted, causing near-empty
#  overlap when intersecting with RNA's gene-symbol rownames)

convert_to_ensembl <- function(df) {
    symbols <- rownames(df)
    mapping <- select(org.Hs.eg.db, keys = symbols, columns = "ENSEMBL", keytype = "SYMBOL")
    mapping <- mapping[!is.na(mapping$ENSEMBL), ]
    mapping <- mapping[!duplicated(mapping$SYMBOL), ]
    mapping <- mapping[!duplicated(mapping$ENSEMBL), ]
    df <- df[mapping$SYMBOL, ]
    rownames(df) <- mapping$ENSEMBL
    df
}

rna  <- convert_to_ensembl(rna)
cat("RNA after ID conversion:", nrow(rna), "genes\n")

meth <- convert_to_ensembl(meth)
...
model_opts$num_factors <- 10   # NOTE: if sample count is small, MOFA will warn;
                                 # consider lowering to ~5 if this keeps warning
